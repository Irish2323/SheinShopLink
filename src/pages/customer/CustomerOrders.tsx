...
import { Package, ShoppingBag, X } from '../../components/icons'
    ...
    <div>
      ...
      {myOrders.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-3 font-semibold text-ink-700">No orders yet</p>
          <p className="text-sm text-ink-400">Place your first order from the shop.</p>
        ...
        <div className="space-y-4">
          {myOrders.map((o) => {
            ...
            const orderTotal = o.items.reduce((s, it) => s + it.unitPrice * it.quantity, 0)
            return (
              <div key={o.id} className="card overflow-hidden">
                <button
                  onClick={() => setExpanded(isOpen ? null : o.id)}
                  className="flex w-full flex-wrap items-center justify-between gap-3 px-5 py-4 text-left transition hover:bg-ink-50/60"
                >
                  <div>
                  ...
                  <div className="flex items-center gap-4">
                    <p className="text-sm font-extrabold text-ink-900">{peso(orderTotal)}</p>
                ...
                {o.rejectedReason && (
                  <div className="mx-5 mb-3 rounded-xl bg-red-50 p-3 text-sm text-red-700 ring-1 ring-red-200">
                    <span className="font-bold">Note: </span>{o.rejectedReason}
                  </div>
                )}
                ...
                {isOpen && (
                  <div className="border-t border-ink-100 bg-ink-50/40 px-5 py-4">
                      ...
                      {o.items.map((it, i) => (
                        <div key={i} className="flex items-center justify-between gap-3 py-2 text-sm">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs font-semibold text-ink-500">{peso(it.unitPrice * it.quantity)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="mt-3 flex justify-between border-t border-ink-100 pt-3 text-sm font-bold">
                      ...
                      <span className="text-ink-900">{peso(orderTotal)}</span>
                    </div>
                    {o.status === 'pending' && (
                      <button
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <button
              onClick={() => {
                if (cancelId) setOrderStatus(cancelId, 'rejected', cancelReason.trim() || 'Cancelled by customer')
                setCancelId(null)
                setCancelReason('')
              }}
      ...
      >
        ...
        <div className="mt-3">
          ...
          <input
            className="input"
            placeholder="e.g. Found a similar item elsewhere"
            value={cancelReason}
