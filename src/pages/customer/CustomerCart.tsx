...
export default function CustomerCart() {
      ...
      cart
        .map((c) => {
          ...
          return {
            ...
            lineTotal: product.regularPrice * c.quantity,
          }
        })
        .filter((x): x is NonNullable<typeof x> => x !== null),
    [cart, products, currentUserId],
  )
  ...
  return (
    <div>
      <PageHeader
        title="My Cart"
        subtitle="Review your items, then place your order"
        actions={<Link to="/" className="btn-secondary">Keep shopping</Link>}
      />
      ...
      {lines.length === 0 ? (
        <div className="card flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-ink-400">Browse the shop and add items to get started.</p>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          ...
          <div className="card h-fit p-5">
            <h2 className="mb-4 font-bold text-ink-900">Order Summary</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-ink-500">Subtotal ({lines.reduce((n, l) => n + l.quantity, 0)} items)</span>
              ...
              <div className="flex justify-between border-t border-ink-100 pt-2">
                ...
                <span className="text-lg font-extrabold text-ink-900">{peso(total)}</span>
              </div>
            </div>
            <div className="mt-4">
              <label className="label">Order notes (optional)</label>
              <textarea className="input min-h-[80px] resize-y" placeholder="Any special instructions…" value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
            <button onClick={submit} className="btn-primary mt-4 w-full">
              <ShoppingBag size={16} /> Place order
            </button>
            <p className="mt-2 text-center text-[11px] text-ink-400">We'll confirm your order shortly.</p>
          </div>
        </div>
      )}
    </div>
